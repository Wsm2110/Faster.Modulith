...
namespace Faster.Modulith.Analyzers.Generators
{
    ...
    public static class Scanner
    {
        ...
        /// <returns>The fully qualified type name or "void" if the symbol is null.</returns>
        ...
        /// </summary>
        /// <param name="symbol">The generic type symbol.</param>
        /// <returns>The unbound generic type name.</returns>
        ...
        public static (List<HandlerInfo> Items, bool HasManualPipelines) ScanTypes(Compilation compilation, CancellationToken ct)
        {
            ...
            foreach (var tree in compilation.SyntaxTrees)
            {
                ...
                // Fast-bail: Only parse the syntax tree if the raw text contains the required keywords
                var sourceText = tree.GetText(ct).ToString();
                if (!sourceText.Contains("AddInfrastructure") || !sourceText.Contains("IPipelineBehavior"))
                {
                    continue;
                }
                ...
                foreach (var method in methods)
                {
                    ...
                    if (bodyNodes != null)
                    {
                        ...
                        if (distinctUsage)
                        {
                            ...
                            break; // Optimization: Stop searching this tree once found
                        }
                    }
                }
            }

            var allSymbols = GetAllNamedTypes(compilation.GlobalNamespace, ct);
            ScanSymbols(allSymbols, results, ct);
        ...
        /// </summary>
        /// <param name="assembly">The assembly symbol to scan.</param>
        /// <param name="ct">The cancellation token.</param>
        /// <returns>A list of discovered artifacts.</returns>
        public static List<HandlerInfo> ScanAssembly(IAssemblySymbol assembly, CancellationToken ct = default)
        {
            var results = new List<HandlerInfo>();
            var allSymbols = GetAllNamedTypes(assembly.GlobalNamespace, ct);
        ...
        /// <summary>
        /// Performs symbol inspection and artifact classification in a single unified pass.
        /// </summary>
            ...
            CancellationToken ct)
        {
            foreach (var symbol in allSymbols)
            {
                ...
                // Retrieve interfaces once per symbol to prevent multiple underlying enumerations
                var interfaces = symbol.AllInterfaces;

                bool isPipelineProcessed = false;
                bool isUseCaseDefinitionProcessed = false;

                foreach (var iface in interfaces)
                {
                    // 1. Pipeline behaviors
                    if (!isPipelineProcessed && iface.Name == "IPipelineBehavior")
                    {
                    ...
                    // 2. UseCase definitions
                    if (!isUseCaseDefinitionProcessed && iface.IsGenericType && iface.Name == "IUseCase")
                    {
                        AddDefinition(results, symbol, iface.TypeArguments[0], ArtifactType.UseCase);
                        isUseCaseDefinitionProcessed = true;
                    }

                    // 3. Command / UseCase / Event handlers
                    if (iface.IsGenericType && iface.Name != "IUseCase")
                    {
                        ...
                        if (handlerType != null && iface.TypeArguments.Length > 0)
                        {
                            ...
                            // Use requestSymbol as the source to accurately map Request/Command constructor properties
                            var handler = Create(
                            ...
                            {
                                AddDefinition(results, (INamedTypeSymbol)requestSymbol, responseSymbol, ArtifactType.UseCase);
                            }
                        }
                    }
        ...
        private static IEnumerable<INamedTypeSymbol> GetAllNamedTypes(INamespaceSymbol ns, CancellationToken ct)
        {
            ...
            {
                foreach (var type in GetAllNamedTypes(subNs, ct))
                    yield return type;
            }
        }
