...
public sealed class DispatchGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(SourceProductionContext context, List<HandlerInfo> items, bool hasManualPipelines, string assemblyName)
    {
        ...
        else if (isModuleProject)
        {
            ...
            // Always generate Dispatcher and Registration, even if lists are empty (as requested on 2026-01-08)
            GenerateInternalDispatcher(context, commands, events, moduleName, assemblyName, hasManualPipelines);
            GenerateModuleRegistration(context, handlers, moduleName, assemblyName);
        }
    }

    private static void GenerateInternalDispatcher(SourceProductionContext context, List<HandlerInfo> commandHandlers, List<HandlerInfo> events, string moduleName, string assemblyName, bool hasManualPipelines)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine($"// Generated on: {DateTime.Now:O}");
        ...
        sb.AppendLine($@"
namespace {assemblyName}.Infrastructure;
...
");
    ...
    private static void GenerateModuleRegistration(
            SourceProductionContext context,
            List<HandlerInfo> handlers,
            string moduleName,
            string targetNamespace) // <--- This holds "Module.Kitchen"
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine($"// Generated on: {DateTime.Now:O}");
        ...
        // 1. CRITICAL: Use the Module's Namespace
        sb.AppendLine($@"
namespace {targetNamespace}.Infrastructure;

// Generated Options POCO
public partial class {moduleName}Options
{{
}}
...
{{
    public static IServiceCollection Add{moduleName}Module(
        ...
        Action<{moduleName}Options> configure = null) 
    {{
        // 1. Configure Options
        if (configure != null)
        {{
        ...
        // 2. Register Dispatcher & Module Interface
        // Note: These are local to this namespace now, so no 'global::' needed
        services.AddScoped<I{moduleName}Dispatcher, {moduleName}Dispatcher>();
        {(handlers.Any(h => h.Type == ArtifactType.UseCase) ? $"services.AddScoped<I{moduleName}Module, {moduleName}Module>();" : "")}
...
");
        ...
        {
            // Framework contracts (IUseCaseHandler, etc.) still need global::Faster.Modulith...
            string interfaceName = h.Type switch
            {
                ...
                ArtifactType.UseCase => $"global::Faster.Modulith.Contracts.IUseCaseHandler<{h.ServiceInterfaceType}, {h.ResponseType}>", // Updated per your earlier requirement
